...
class sigmoid_DBN_frozen(object):
...
#         print ('Accuracy%:', accuracy.eval({self._a[0]: X_test, self.y: y_test})*100)
    ...
    def predict_ex(self,X,Y):
        ...
        frozen_f1_y = tf.argmax(self.y,1)
        frozen_f1_ye = frozen_f1_y.eval({self.y:Y})
        frozen_f1_p = tf.argmax(self._a[-1],1)
        frozen_f1_pe = frozen_f1_p.eval({self._a[0]:X})
        frozen_precision = metrics.precision_score(frozen_f1_ye, frozen_f1_pe)
        frozen_recall = metrics.recall_score(frozen_f1_ye, frozen_f1_pe)
        frozen_f1 = metrics.f1_score(frozen_f1_ye, frozen_f1_pe)
        
        return pred_acc, frozen_precision, frozen_recall, frozen_f1
