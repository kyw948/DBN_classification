...
class dbn:
	...
	def __init__(self,nvisible,nhidden=[100,100,50],nlabels=0,eta=0.001,momentum=0.5,nCDsteps=1,nepochs=1000,useprobstate=0):
	...
	def classify(self,inputs,labels):
		...
		ex1 = labels.argmax(axis=1).sum()
		ex2 = l.argmax(axis=1).sum()
		ex3 = (l.argmax(axis=1) != labels.argmax(axis=1)).sum()
		ex4 = (ex1+ex2)
		ex5 = ex3/ex2
		ex6 = ex3/ex4
		print ('ErrorsPer : ', ex5)
		print ('ErrorsPer : ', ex6)
		train_acc = np.mean(l.argmax(axis=1) == labels.argmax(axis=1))
		print ('Error : ', train_acc)
...
def test_dbn_digs():
	...
	pl.figure(), pl.imshow(pobs[0,:].reshape(20,16),cmap=pl.cm.gray,interpolation='nearest')
	pl.figure(), pl.imshow(pobs[1,:].reshape(20,16),cmap=pl.cm.gray,interpolation='nearest')
	pl.figure(), pl.imshow(pobs[2,:].reshape(20,16),cmap=pl.cm.gray,interpolation='nearest')
	pl.figure(), pl.imshow(pobs[3,:].reshape(20,16),cmap=pl.cm.gray,interpolation='nearest')
	return v, l, obs, pobs
